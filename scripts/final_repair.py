...
if "app.post('/api/contacts'" not in s:
    ...
    if insert_at >= 0:
        s = s[:insert_at] + contact_route + s[insert_at:]
    else:
        raise SystemExit('CONTACT_ROUTE_ANCHOR_NOT_FOUND')
if "app.post('/api/messages',auth" in s and "linkContacts(req.user.sub,u.id);" not in s:
    s = s.replace(
        "const sender=db.prepare('SELECT id,wethaq_id,name FROM users WHERE id=?').get(req.user.sub);",
