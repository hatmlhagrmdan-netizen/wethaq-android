...
helper=anchor+"function linkContacts(a,b){db.prepare('INSERT OR IGNORE INTO contacts(user_id,contact_id) VALUES(?,?)').run(a,b);db.prepare('INSERT OR IGNORE INTO contacts(user_id,contact_id) VALUES(?,b)'.replace('?,b','?,?)').replace('VALUES(?,?)','VALUES(?,?)').replace('b)', 'b)'))"
# Add automatic bidirectional contacts without replacing the media send implementation.
