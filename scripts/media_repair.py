...
old='if(r==703)uploadAvatar(b64,"image/jpeg");else sendImage(b64,"image/jpeg");'
new='if(r==703)uploadAvatar(b64,"image/jpeg");else{pendingImageData=b64;pendingAudioData=null;pendingMime="image/jpeg";pendingMediaType=1;showImagePreview(b64);}'
s=s.replace(old,new)
old='private void sendImage(String data,String mime){if(activeId==null)return;'
new='private void showImagePreview(String data){try{byte[] z=Base64.getDecoder().decode(data);Bitmap bm=BitmapFactory.decodeByteArray(z,0,z.length);if(input!=null){input.setCompoundDrawablesWithIntrinsicBounds(new BitmapDrawable(getResources(),bm),null,null,null);input.setHint("الصورة جاهزة للإرسال — اضغط إرسال");}}catch(Exception ignored){}}\n private void sendImage(String data,String mime){if(activeId==null)return;'
s=s.replace(old,new)
