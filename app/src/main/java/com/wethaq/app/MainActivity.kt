...
class MainActivity : AppCompatActivity() {
    ...
    private fun search(value:String){val q=value.trim().removePrefix("@");if(q.length<2){toast("اكتب اسمًا أو معرفًا");return};request("GET","/api/search?q=${URLEncoder.encode(q,"UTF-8")}",null,false){ok,body->runOnUiThread{if(ok)showResults(body.optJSONArray("users")?:JSONArray())else toast("تعذر البحث الآن")}}}
    private fun showResults(arr:JSONArray){val r=root();val sc=ScrollView(this);val c=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL;setPadding(dp(14),dp(8),dp(14),dp(20))};add(c,header("نتائج البحث","اختر الشخص لإضافته أو مراسلته"){home()});if(arr.length()==0)add(c,center("لم يتم العثور على المستخدم",16f,gray),80);for(i in 0 until arr.length()){val u=arr.optJSONObject(i)?:continue;val id=u.optString("wethaq_id");val name=u.optString("name").ifBlank{"مستخدم وَثاق"};val item=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL;background=card();setPadding(dp(14),dp(10),dp(14),dp(10))};add(item,txt(name,17f,navy,true),30);add(item,txt("@$id",14f,teal,true),28);add(item,button("＋ حفظ جهة الاتصال"){saveContact(u);contacts()},48,0f,4);add(item,outline("✉ مراسلة"){openChat(id,name)},44,0f,3);add(c,item,0,1f,5)};sc.addView(c);add(r,sc,0,1f);setContentView(r)}

    private fun saveContact(user:JSONObject){val id=user.optString("wethaq_id");if(id.isBlank())return;val normalized=JSONObject().put("wethaq_id",id).put("name",user.optString("name").ifBlank{"مستخدم وَثاق"});val contacts=JSONArray(prefs.getString("contacts","[]")?:"[]");var found=false;for(i in 0 until contacts.length())if(contacts.optJSONObject(i)?.optString("wethaq_id")==id)found=true;if(!found)contacts.put(normalized);prefs.edit().putString("contacts",contacts.toString()).apply();if(token.isNotBlank()&&hasNetwork())request("POST","/api/contacts",JSONObject().put("wethaqId",id),true){_,_->};toast("تم حفظ ${normalized.optString("name")}\n@$id")}
    private fun contacts(){val r=root();val sc=ScrollView(this);val c=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL;setPadding(dp(14),dp(8),dp(14),dp(20))};add(c,header("جهات الاتصال","الاسم والمعرف محفوظان معًا"){home()});val local=JSONArray(prefs.getString("contacts","[]")?:"[]");if(local.length()==0)add(c,center("لا توجد جهات اتصال محفوظة",16f,gray),70);for(i in 0 until local.length()){val u=local.optJSONObject(i)?:continue;val id=u.optString("wethaq_id");val name=u.optString("name").ifBlank{"مستخدم وَثاق"};val row=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL;background=card();setPadding(dp(14),dp(9),dp(14),dp(9))};add(row,txt(name,17f,navy,true),30);add(row,txt("@$id",14f,teal,true),28);add(row,button("✉ مراسلة"){openChat(id,name)},46,0f,4);add(c,row,0,1f,5)};sc.addView(c);add(r,sc,0,1f);setContentView(r);if(token.isNotBlank()&&hasNetwork())pullContacts()}
    ...
    private fun openChat(id:String,name:String){val r=root();val sc=ScrollView(this);val c=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL;setPadding(dp(12),dp(6),dp(12),dp(10))};add(c,header(name,"@$id"){contacts()});val box=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL};add(c,box,0,1f);val input=field("اكتب رسالتك...");add(c,input,55,0f,3);add(c,button("➤ إرسال"){val body=input.text.toString().trim();if(body.isBlank())toast("اكتب رسالة أولًا")else{input.setText("");addBubble(box,body,true,"جاري الإرسال");sendMessage(to=id,body=body,box=box)}},52,0f,2);sc.addView(c);add(r,sc,0,1f);setContentView(r);loadMessages(id,box)}
    private fun addBubble(box:LinearLayout,body:String,mine:Boolean,status:String){val v=TextView(this).apply{text="$body\n$status";textSize=15f;setTextColor(if(mine)Color.WHITE else navy);background=android.graphics.drawable.GradientDrawable().apply{setColor(if(mine)teal else Color.WHITE);cornerRadius=dp(14).toFloat();setStroke(dp(1),line)};setPadding(dp(13),dp(9),dp(13),dp(9))};val lp=LinearLayout.LayoutParams(-2,-2);lp.gravity=if(mine)Gravity.END else Gravity.START;lp.setMargins(dp(5),dp(4),dp(5),dp(4));box.addView(v,lp)}
    private fun loadMessages(id:String,box:LinearLayout){if(!hasNetwork()||token.isBlank())return;request("GET","/api/messages/${URLEncoder.encode(id,"UTF-8")}",null,true){ok,body->if(ok)runOnUiThread{box.removeAllViews();val arr=body.optJSONArray("messages")?:JSONArray();for(i in 0 until arr.length()){val m=arr.optJSONObject(i)?:continue;addBubble(box,m.optString("body"),m.optString("sender_wethaq_id")==myId,m.optString("status","مستلمة"))}}}}
    private fun sendMessage(to:String,body:String,box:LinearLayout){if(!hasNetwork()||token.isBlank()){queueMessage(to,body);toast("حُفظت الرسالة وسترسل عند عودة الإنترنت");return};request("POST","/api/messages",JSONObject().put("to",to).put("body",body),true){ok,_->runOnUiThread{if(ok)replaceLastStatus(box,"تم الإرسال")else{replaceLastStatus(box,"محفوظة للإرسال");queueMessage(to,body);toast("تعذر الإرسال الآن، حُفظت الرسالة للإرسال لاحقًا")}}}}
    private fun replaceLastStatus(box:LinearLayout,status:String){if(box.childCount==0)return;val v=box.getChildAt(box.childCount-1) as? TextView?:return;val old=v.text.toString().substringBeforeLast("\n");v.text="$old\n$status"}
    ...
    private fun flushPendingMessages(){if(!hasNetwork()||token.isBlank())return;val old=JSONArray(prefs.getString("pending","[]")?:"[]");if(old.length()==0)return;val remaining=JSONArray();for(i in 0 until old.length()){val m=old.optJSONObject(i)?:continue;request("POST","/api/messages",JSONObject().put("to",m.optString("to")).put("body",m.optString("body")),true){ok,_->if(!ok)synchronized(remaining){remaining.put(m)}}};prefs.edit().putString("pending",remaining.toString()).apply()}
