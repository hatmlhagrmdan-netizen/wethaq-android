...
/** Shared Wethaq visual layer: premium RTL styling, avatars and founder-brand motion. */
public final class WethaqUi{
 private static final int GOLD=Color.rgb(212,175,55),BRIGHT_GOLD=Color.rgb(255,220,110),DARK=Color.rgb(28,28,32),PRESSED=Color.rgb(55,45,24);
 ...
 public static void apply(Application app,Activity a){
  ...
  if(r instanceof ViewGroup)styleTree(a,(ViewGroup)r);
 }
 ...
 private static void styleButton(Button b){
  ...
  b.setTypeface(Typeface.DEFAULT,Typeface.BOLD);b.setMinHeight(dp(b,68));b.setMinimumHeight(dp(b,68));
  b.setMinWidth(0);b.setMinimumWidth(0);b.setPadding(dp(b,14),0,dp(b,14),0);b.setGravity(Gravity.CENTER);
  StateListDrawable s=new StateListDrawable();
  s.addState(new int[]{android.R.attr.state_pressed},face(b,PRESSED,2));
  s.addState(new int[]{-android.R.attr.state_enabled},face(b,Color.rgb(45,45,45),1));
  s.addState(new int[]{},face(b,DARK,3));b.setBackground(s);b.setElevation(dp(b,7));b.setTranslationZ(dp(b,1));
 ...
 private static void styleText(Activity a,TextView t){
  if(t.getText()==null)return;t.setIncludeFontPadding(true);if(t.getTextSize()<dp(t,16))t.setTextSize(16);if(t.getGravity()==0)t.setGravity(Gravity.CENTER_VERTICAL|Gravity.RIGHT);
  String s=t.getText().toString().trim();if(s.isEmpty())return;if(Boolean.TRUE.equals(t.getTag()))return;
  if(s.contains(FOUNDER_NAME)||s.contains("مدير ومؤسس وثاق"))animateFounderBrand(t);
