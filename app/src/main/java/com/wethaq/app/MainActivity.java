...
public final class MainActivity extends Activity {
    ...
    private void login(){base("وَثاق");content.addView(tv("هويتك الرقمية الآمنة",24,Color.WHITE));EditText n=field("الاسم الثلاثي"),y=field("سنة الميلاد");y.setInputType(2);content.addView(n,lp(-1,-2,0,8));content.addView(y,lp(-1,-2,0,8));Button a=btn("دخول"),c=btn("إنشاء هوية جديدة");content.addView(a);content.addView(c);a.setOnClickListener(v->auth(n.getText().toString().trim(),y.getText().toString().trim(),false));c.setOnClickListener(v->auth(n.getText().toString().trim(),y.getText().toString().trim(),true));content.addView(tv("المؤسس: حاتم حسين الحاج رمضان\nWethaq "+VERSION,14,Color.LTGRAY));}
    ...
    private void loadMessages(){if(activeId==null||!isOnline())return;final String target=activeId;io.execute(()->{try{HttpResult r=request("GET","/api/messages/"+URLEncoder.encode(target,"UTF-8"),null,auth());if(r.code==200){JSONArray a=new JSONObject(r.body).optJSONArray("messages");main.post(()->{if(target.equals(activeId))renderMessages(a);});}else if(r.code==401){main.post(()->{prefs.edit().clear().apply();stopPolling();login();});}}catch(Exception ignored){}});}
    private void renderMessages(JSONArray a){if(messages==null)return;messages.removeAllViews();if(a==null){messages.addView(tv("ابدأ المحادثة برسالة جديدة",16,Color.LTGRAY));return;}for(int i=0;i<a.length();i++){JSONObject m=a.optJSONObject(i);if(m!=null){String body=m.optString("body");String status=m.optString("status","");messages.addView(tv(status.isEmpty()?body:body+"\n"+status,16,Color.WHITE));}}}
    private void sendMessage(){if(input==null)return;final String text=input.getText().toString().trim();final String target=activeId;if(text.isEmpty()||target==null||target.isEmpty())return;if(target.equals(prefs.getString(USER_ID,""))){toast("لا يمكن الإرسال إلى نفسك");return;}if(!isOnline()){toast("لا يوجد اتصال بالإنترنت");return;}input.setEnabled(false);io.execute(()->{try{JSONObject b=new JSONObject();b.put("receiver_id",target);b.put("to",target);b.put("body",text);HttpResult r=request("POST","/api/messages",b.toString(),auth());main.post(()->{input.setEnabled(true);if(r.code>=200&&r.code<300){input.setText("");loadMessages();toast("تم إرسال الرسالة ✓");}else if(r.code==401){prefs.edit().clear().apply();stopPolling();login();}else toast(error(r));});}catch(Exception e){main.post(()->{input.setEnabled(true);toast("فشل الإرسال");});}});}
    ...
    private void pollInbox(){if(!hasToken()||!isOnline())return;io.execute(()->{try{request("GET","/api/messages/inbox",null,auth());}catch(Exception ignored){}});}
