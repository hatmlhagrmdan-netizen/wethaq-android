...
public final class PublicAdministrationActivity extends Activity {
    ...
    private void show(String raw){try{JSONObject o=new JSONObject(raw);body.removeAllViews();body.addView(card("💳 نظام الاشتراك الإداري\nالمدة: سنة واحدة\n\n🥇 النائب الأول: 20$ سنويًا\nالصلاحيات: الإشراف الإداري، إدارة أعضاء الإدارة، واتخاذ إجراءات الحظر ضمن حدود الرتبة.\n\n🥈 النائب الثاني: 17$ سنويًا\nالصلاحيات: الإشراف الإداري والمساعدة في إدارة أعضاء الإدارة واتخاذ إجراءات الحظر ضمن حدود الرتبة.\n\n🥉 النائب الثالث: 12$ سنويًا\nالصلاحيات: صلاحيات إدارية محددة واتخاذ الإجراءات المسموح بها للرتبة.\n\n👤 عضو الإدارة: 8$ سنويًا\nالصلاحيات: المشاركة في الإدارة وتنفيذ الإجراءات المسموح بها لعضو الإدارة.\n\n🔐 حماية المؤسس: منصب المؤسس محمي ولا يخضع للحظر أو الإزالة من أي رتبة إدارية أخرى.\n\n📩 للراغبين في الحصول على منصب إداري: التواصل مباشرةً مع المدير العام والمؤسس لطلب المنصب واستكمال إجراءات الاشتراك.",true));body.addView(card("📋 الأسعار والصلاحيات تخضع للرتبة المسجلة في النظام، ومدة الصلاحية سنة واحدة من تاريخ التفعيل.",false));JSONObject f=o.optJSONObject("founder");body.addView(card("👑 المؤسس\n"+(f==null?"غير محدد":f.optString("name","غير محدد")),true));JSONObject d=o.optJSONObject("deputies");body.addView(card("🥇 النائب الأول\n"+nameOf(d==null?null:d.optJSONObject("deputy1")),false));body.addView(card("🥈 النائب الثاني\n"+nameOf(d==null?null:d.optJSONObject("deputy2")),false));body.addView(card("🥉 النائب الثالث\n"+nameOf(d==null?null:d.optJSONObject("deputy3")),false));body.addView(text("👥 أعضاء الإدارة — 10 خانات",20,Color.rgb(212,175,55)));JSONArray m=o.optJSONArray("members");for(int i=0;i<10;i++){JSONObject x=null;if(m!=null&&i<m.length())x=m.optJSONObject(i);String s=(i+1)+". "+(x==null?"غير معين":x.optString("name","غير معروف"))+"\nسنة الميلاد: "+(x==null?"—":x.optString("birth_year","—"))+"\nالرمز الشخصي: سري";body.addView(card(s,false));}body.addView(card("المستخدمون\nجميع المستخدمين خارج الهيكل الإداري",false));}catch(Exception e){status.setText("🔴 حدث خطأ أثناء قراءة البيانات");}}
