...
public final class PublicAdministrationActivity extends Activity {
    ...
    private void build(){LinearLayout root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setPadding(dp(14),dp(14),dp(14),dp(14));root.setBackgroundColor(Color.BLACK);TextView title=text("👥 وَثاق — الإدارة العامة",23,Color.rgb(212,175,55));title.setGravity(Gravity.CENTER);title.setTypeface(Typeface.DEFAULT,Typeface.BOLD);root.addView(title,new LinearLayout.LayoutParams(-1,dp(72)));Button access=button("🔐 دخول الإدارة / تفعيل المنصب");root.addView(access,new LinearLayout.LayoutParams(-1,dp(64)));access.setOnClickListener(v->startActivity(new Intent(this,AdminAccessActivity.class)));ScrollView sc=new ScrollView(this);body=new LinearLayout(this);body.setOrientation(LinearLayout.VERTICAL);body.addView(card("⏳ جاري تحميل شاغلي المناصب الحاليين من خادم وَثاق...",true));body.addView(card("👑 المؤسس\nالعدد: 1\nالصلاحيات: جميع الصلاحيات، وإدارة وتعيين المناصب.",false));body.addView(card("🏛 المدير التنفيذي — 100$ سنويًا\nالعدد: 1",false));body.addView(card("🥇 النائب الأول — 70$ سنويًا\nالعدد: 1",false));body.addView(card("🥈 النائب الثاني — 50$ سنويًا\nالعدد: 1",false));body.addView(card("🥉 النائب الثالث — 30$ سنويًا\nالعدد: 1",false));body.addView(card("🛡 المشرف — 15$ سنويًا\nالعدد: 9",false));body.addView(card("👥 عضو الإدارة — 7$ سنويًا\nالعدد: 27",false));body.addView(card("⭐ عضو مميز — 2$ سنويًا",false));body.addView(card("🔐 حماية السجلات: رموز الدخول الشخصية والرموز السرية لا تظهر للعامة. تظهر فقط حالة حماية الرمز، بينما تبقى القيم السرية غير قابلة للعرض.",true));sc.addView(body);root.addView(sc,new LinearLayout.LayoutParams(-1,0,1));Button back=button("رجوع");root.addView(back,new LinearLayout.LayoutParams(-1,dp(64)));back.setOnClickListener(v->finish());setContentView(root);}
