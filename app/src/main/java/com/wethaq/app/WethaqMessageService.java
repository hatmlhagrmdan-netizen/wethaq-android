...
public final class WethaqMessageService extends Service {
    ...
    private void syncPendingNotifications(){
        final String token=WethaqSession.get(this,"token");
    ...
    private void acknowledgeMessages(java.util.ArrayList<Integer> ids){if(ids==null||ids.isEmpty())return;String token=WethaqSession.get(this,"token");if(token.length()<10)return;try{org.json.JSONArray a=new org.json.JSONArray();for(Integer id:ids)a.put(id);JSONObject body=new JSONObject();body.put("ids",a);Request r=new Request.Builder().url(WethaqConfig.API+"/api/messages/notifications/delivered").header("Authorization","Bearer "+token).header("Content-Type","application/json").post(RequestBody.create(body.toString(),MediaType.parse("application/json"))).build();client.newCall(r).enqueue(new Callback(){@Override public void onFailure(Call c,java.io.IOException e){}@Override public void onResponse(Call c,Response r){r.close();}});}catch(Exception ignored){}}
    private void acknowledgeActions(java.util.ArrayList<Integer> ids){if(ids==null||ids.isEmpty())return;String token=WethaqSession.get(this,"token");if(token.length()<10)return;try{org.json.JSONArray a=new org.json.JSONArray();for(Integer id:ids)a.put(id);JSONObject body=new JSONObject();body.put("ids",a);Request r=new Request.Builder().url(WethaqConfig.API+"/api/me/actions/delivered").header("Authorization","Bearer "+token).header("Content-Type","application/json").post(RequestBody.create(body.toString(),MediaType.parse("application/json"))).build();client.newCall(r).enqueue(new Callback(){@Override public void onFailure(Call c,java.io.IOException e){}@Override public void onResponse(Call c,Response r){r.close();}});}catch(Exception ignored){}}
