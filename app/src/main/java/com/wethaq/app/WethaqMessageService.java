...
public final class WethaqMessageService extends Service {
    ...
    private void handle(String text){try{JSONObject o=new JSONObject(text);String event=o.optString("event");if("message".equals(event)){JSONObject m=o.optJSONObject("message");String id=m==null?"":m.optString("id","");if(!firstTimeMessage(id))return;String sender=m==null?"مستخدم":m.optString("sender_name","مستخدم");String senderId=m==null?"":m.optString("sender_wethaq_id","");String body=m==null?"رسالة جديدة":m.optString("body","");String type=m==null?"text":m.optString("message_type","text");String content=body.isEmpty()?("audio".equals(type)?"🎙 رسالة صوتية":"image".equals(type)?"🖼 صورة":"رسالة جديدة"):body;showMessage(sender,senderId,content);}else if("call".equals(event)){JSONObject from=o.optJSONObject("from");String id=from==null?"":from.optString("wethaq_id","");String name=from==null?"مستخدم":from.optString("name","مستخدم");String type=o.optString("type","");String payload=o.optString("payload","");if("offer".equals(type)&&!id.isEmpty()){boolean audioOnly=!payload.contains("m=video");showIncomingCall(id,name,audioOnly,payload);}}}catch(Exception ignored){}}
    private void showMessage(String title,String senderId,String body){NotificationCompat.Builder b=new NotificationCompat.Builder(this,MESSAGE_CHANNEL).setSmallIcon(android.R.drawable.ic_dialog_email).setContentTitle(title).setContentText(body).setStyle(new NotificationCompat.BigTextStyle().bigText(body)).setAutoCancel(true).setPriority(NotificationCompat.PRIORITY_MAX).setCategory(NotificationCompat.CATEGORY_MESSAGE).setDefaults(NotificationCompat.DEFAULT_VIBRATE);if(senderId!=null&&!senderId.isEmpty()){Intent i=new Intent(this,ProfessionalConversationActivity.class);i.putExtra("notification_target",senderId);i.putExtra("notification_name",title);i.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK|Intent.FLAG_ACTIVITY_CLEAR_TOP);PendingIntent pi=PendingIntent.getActivity(this,senderId.hashCode(),i,PendingIntent.FLAG_UPDATE_CURRENT|PendingIntent.FLAG_IMMUTABLE);b.setContentIntent(pi);}int id=(int)(System.currentTimeMillis()&0x7fffffff);NotificationManager nm=getSystemService(NotificationManager.class);if(nm!=null)nm.notify(id,b.build());}
