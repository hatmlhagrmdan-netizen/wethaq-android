...
public final class WethaqMessageService extends Service {
    ...
    private void createChannels(){
        ...
        messages.setSound(RingtoneManager.getDefaultUri(RingtoneManager.TYPE_NOTIFICATION),null);
    ...
    private Notification baseNotification(String text){
        return new NotificationCompat.Builder(this,SERVICE_CHANNEL)
            ...
            .setContentTitle("وَثاق").setContentText(text).setOngoing(true)
    ...
    private void connect(){
        ...
        try{
            ...
            socket=client.newWebSocket(r,new WebSocketListener(){
                @Override public void onOpen(WebSocket w,Response x){update("متصل — استقبال الرسائل والمكالمات فعال");}
                @Override public void onMessage(WebSocket w,String text){handle(text);}
                @Override public void onClosed(WebSocket w,int code,String reason){socket=null;scheduleReconnect();}
                @Override public void onFailure(WebSocket w,Throwable t,Response r){socket=null;scheduleReconnect();}
            });
    ...
    private void scheduleReconnect(){
        if(stopping)return;
        new Handler(Looper.getMainLooper()).postDelayed(()->{if(!stopping&&(socket==null))connect();},3000);
    }
    ...
    private void handle(String text){
        try{
            ...
            if("message".equals(event)){
                ...
                if("offer".equals(type)&&!id.isEmpty())showIncomingCall(id,name,false);
            }
    ...
    private void showIncomingCall(String target,String name,boolean audioOnly){
        ...
        i.putExtra("target",target);i.putExtra("name",name);i.putExtra("audioOnly",audioOnly);
        ...
        NotificationCompat.Builder b=new NotificationCompat.Builder(this,CALL_CHANNEL)
            ...
            .setContentTitle("مكالمة واردة من "+name).setContentText("اضغط للرد على المكالمة")
            ...
            .setOngoing(true).setTimeoutAfter(60000);
        NotificationManager nm=getSystemService(NotificationManager.class);
        if(nm!=null)nm.notify(4102,b.build());
    }

    @Override public int onStartCommand(Intent i,int flags,int startId){
        stopping=false;if(socket==null)connect();return START_STICKY;
    }
