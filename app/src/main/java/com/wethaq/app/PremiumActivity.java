...
public final class PremiumActivity extends Activity {
    ...
    private static final String PREF="wethaq_premium";
    ...
    @Override public void onCreate(Bundle b){super.onCreate(b);getWindow().setStatusBarColor(BLACK);getWindow().setNavigationBarColor(BLACK);loadContacts();requestNotifications();showHome();}
    private void requestNotifications(){if(Build.VERSION.SDK_INT>=33&&checkSelfPermission(Manifest.permission.POST_NOTIFICATIONS)!=PackageManager.PERMISSION_GRANTED)requestPermissions(new String[]{Manifest.permission.POST_NOTIFICATIONS},90);}
    ...
    private void showHome(){LinearLayout r=root();TextView top=tv("وَثاق    •    برنامج التواصل الاجتماعي\nالمؤسس حاتم حسين الحاج رمضان",22,GOLD);top.setGravity(Gravity.CENTER);top.setBackground(bg(PANEL,28));r.addView(top,new LinearLayout.LayoutParams(-1,-2));ScrollView s=new ScrollView(this);body=new LinearLayout(this);body.setOrientation(LinearLayout.VERTICAL);s.addView(body);r.addView(s,new LinearLayout.LayoutParams(-1,0,1));
        addMenu("🔎  البحث",v->showSearch());addMenu("👥  جهات الاتصال",v->showContacts());addMenu("⚙️  الإعدادات",v->showSettings());addMenu("🪪  المعلومات الشخصية",v->showPersonal());addMenu("🖼️  المحفوظات",v->showArchive());
        TextView trust=tv("🛡️ مركز الثقة\nهوية رقمية • خصوصية • محادثات عربية حديثة",15,Color.WHITE);trust.setBackground(bg(0xFF101014,28));body.addView(trust,margin(0,16,0,0));
    }
    ...
    private void showChat(String id){currentTarget=id;base("المراسلة",v->showContacts());LinearLayout profile=new LinearLayout(this);profile.setOrientation(LinearLayout.VERTICAL);profile.setGravity(Gravity.CENTER);TextView photo=tv("◉",42,GOLD);photo.setGravity(Gravity.CENTER);profile.addView(photo);TextView name=tv(id,20,Color.WHITE);name.setGravity(Gravity.CENTER);profile.addView(name);profile.setBackground(bg(PANEL,28));body.addView(profile,margin(0,8,0,8));
        ScrollView sc=new ScrollView(this);LinearLayout msgs=new LinearLayout(this);msgs.setOrientation(LinearLayout.VERTICAL);sc.addView(msgs);body.addView(sc,new LinearLayout.LayoutParams(-1,0,1));
        addMessage(msgs,"مرحباً بك في محادثة وَثاق",false);addMessage(msgs,"رسائلك تظهر يمينًا ورسائل الطرف الآخر يسارًا",false);
        LinearLayout bar=new LinearLayout(this);bar.setGravity(Gravity.CENTER_VERTICAL);EditText text=input("اكتب رسالة…");Button mic=btn("🎙️");Button gal=btn("🖼️");Button send=btn("إرسال");bar.addView(text,new LinearLayout.LayoutParams(0,-2,1));bar.addView(mic,new LinearLayout.LayoutParams(58,-2));bar.addView(gal,new LinearLayout.LayoutParams(58,-2));bar.addView(send,new LinearLayout.LayoutParams(-2,-2));body.addView(bar);
        send.setOnClickListener(v->{String m=text.getText().toString().trim();if(m.isEmpty())return;addMessage(msgs,m,true);text.setText("");});mic.setOnClickListener(v->toggleRecord(mic,msgs));gal.setOnClickListener(v->pickImage(msgs));startIncomingNotifications();}
    ...
    private void pickImage(LinearLayout msgs){Intent i=new Intent(Intent.ACTION_OPEN_DOCUMENT);i.setType("image/*");i.addCategory(Intent.CATEGORY_OPENABLE);startActivityForResult(i,72);}
    @Override protected void onActivityResult(int req,int res,Intent data){super.onActivityResult(req,res,data);if(req==72&&res==RESULT_OK&&data!=null&&data.getData()!=null){Uri u=data.getData();gallery.add(u);getPreferences(0).edit().putString("archive_count",String.valueOf(gallery.size())).apply();toast("تمت إضافة الصورة إلى المحفوظات");}}

    private void showSettings(){base("الإعدادات",v->showHome());Switch n=new Switch(this);n.setText("الإشعارات");n.setTextColor(Color.WHITE);n.setTextSize(17);n.setChecked(getPreferences(0).getBoolean("notify",true));Switch s=new Switch(this);s.setText("صوت النقر");s.setTextColor(Color.WHITE);s.setTextSize(17);s.setChecked(getPreferences(0).getBoolean("click",true));body.addView(n);body.addView(s);body.addView(tv("الاسم الشخصي",15,GOLD));EditText name=input("الاسم الثلاثي");EditText birth=input("سنة الميلاد");TextView id=tv("المعرف الشخصي: "+getPreferences(0).getString("id","سيُنشأ عند الحفظ"),15,Color.LTGRAY);Button save=btn("حفظ البيانات");Button photo=btn("🖼️ الصورة الشخصية");body.addView(name,margin(0,8,0,8));body.addView(birth,margin(0,0,0,8));body.addView(id,margin(0,0,0,8));body.addView(save,margin(0,0,0,8));body.addView(photo);n.setOnCheckedChangeListener((v,c)->getPreferences(0).edit().putBoolean("notify",c).apply());s.setOnCheckedChangeListener((v,c)->getPreferences(0).edit().putBoolean("click",c).apply());save.setOnClickListener(v->{String generated=getPreferences(0).getString("id","");if(generated.isEmpty())generated="WTH-"+(System.currentTimeMillis()%1000000);getPreferences(0).edit().putString("name",name.getText().toString()).putString("birth",birth.getText().toString()).putString("id",generated).apply();id.setText("المعرف الشخصي: "+generated);toast("تم حفظ المعلومات ✓");});photo.setOnClickListener(v->pickProfile());}
