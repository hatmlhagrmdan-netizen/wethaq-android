...
const original=s;
function replaceOnce(a,b,label){if(!s.includes(a)){if(s.includes(b))return;throw new Error(`hardening pattern missing: ${label}`);}s=s.replace(a,b);}
function insertOnce(anchor,insert,label){if(s.includes(insert.trim()))return;if(!s.includes(anchor))throw new Error(`hardening anchor missing: ${label}`);s=s.replace(anchor,insert+anchor);}

const capabilityCode=`const WETHAQ_CAPABILITIES_V2=${JSON.stringify({
...
})};\nconst __CAP_GUARD=(name)=>(req,res,next)=>{const c=WETHAQ_CAPABILITIES_V2[req.adminRow?.role]||{};if(!c[name])return res.status(403).json({error:'capability_'+name+'_denied'});next();};\n`;
insertOnce("const app=express(),server=http.createServer(app),wss=new WebSocketServer({server,path:'/ws'});\n",capabilityCode,'early capability definitions');
...
replaceOnce("app.post('/api/admin/rbac/ban',adminAuth,(req,res)=>{","app.post('/api/admin/rbac/ban',adminAuth,__CAP_GUARD('manageUsers'),(req,res,next)=>{const mins=Math.max(0,Number(req.body?.minutes||0));const c=WETHAQ_CAPABILITIES_V2[req.adminRow.role]||{};if(mins>0&&!c.banTemporary)return res.status(403).json({error:'temporary_ban_denied'});if(mins===0&&!c.banPermanent)return res.status(403).json({error:'permanent_ban_denied'});next();},(req,res)=>{",'ban capabilities');
