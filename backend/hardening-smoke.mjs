...
const badName = await req('/api/admin/assign',{method:'POST',headers:{authorization:`Bearer ${login.body.token}`},body:JSON.stringify({wethaqId:member.body.user.wethaq_id,name:memberName+' خطأ',birthYear:1998,personalCode:memberCode,role:'admin_member'})});
assert(badName.r.status===403 && badName.body.error==='assignment_identity_mismatch','assignment name mismatch bypassed');
const badBirth = await req('/api/admin/assign',{method:'POST',headers:{authorization:`Bearer ${login.body.token}`},body:JSON.stringify({wethaqId:member.body.user.wethaq_id,name:memberName,birthYear:1997,personalCode:memberCode,role:'admin_member'})});
...
assert((await req('/api/admin/structure',{headers:{authorization:`Bearer ${memberLogin.body.token}`}})).r.status===200,'public structure stopped working for admin member');
