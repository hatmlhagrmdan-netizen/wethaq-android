...
function requireAdmin(req,res,next){const header=req.headers.authorization||'',token=header.startsWith('Bearer ')?header.slice(7):'';if(!token||JWT_SECRET.length<32)return res.status(401).json({error:'admin_unauthorized'});try{const claims=jwt.verify(token,JWT_SECRET);if(claims.admin!==true)return res.status(403).json({error:'admin_token_required'});const row=activeRole(Number(claims.sub));if(!row||row.role!==claims.role)return res.status(403).json({error:'admin_role_inactive'});if(Number(row.code_version||1)!==Number(claims.codeVersion||1))return res.status(403).json({error:'admin_session_revoked'});req.admin=claims;req.adminRow=row;next();}catch{return res.status(401).json({error:'invalid_admin_token'});}}
function assignmentIdentityGuard(req,res,next){const targetId=String(req.body?.wethaqId||'').trim(),name=safeName(req.body?.name),birthYear=Number(req.body?.birthYear),personalCode=String(req.body?.personalCode||'').trim();if(!targetId||!name||!Number.isInteger(birthYear)||!/^\d{6,12}$/.test(personalCode))return res.status(400).json({error:'assignment_identity_required'});const target=db.prepare('SELECT id,name,birth_year,wethaq_id,personal_code_hash FROM users WHERE wethaq_id=?').get(targetId);if(!target)return res.status(404).json({error:'user_not_found'});if(safeName(target.name)!==name||Number(target.birth_year)!==birthYear)return res.status(403).json({error:'assignment_identity_mismatch'});if(!verifyPersonalCode(personalCode,target.personal_code_hash))return res.status(403).json({error:'assignment_personal_code_invalid'});delete req.body.personalCode;next();}
function nonFounderTargetGuard(req,res,next){const targetId=String(req.body?.wethaqId||'').trim(),target=db.prepare('SELECT wethaq_id FROM users WHERE wethaq_id=?').get(targetId);if(target?.wethaq_id===OWNER_WETHAQ_ID)return res.status(403).json({error:'founder_protected'});next();}
function targetManagementGuard(req,res,next){const targetId=String(req.body?.wethaqId||'').trim(),target=db.prepare('SELECT id,wethaq_id FROM users WHERE wethaq_id=?').get(targetId);if(!target)return res.status(404).json({error:'user_not_found'});const tr=activeRole(target.id),targetRole=tr?.role||'user',actorRank=RANK[req.adminRow?.role]??RANK.user,targetRank=RANK[targetRole]??RANK.user;if(targetRole==='founder'||actorRank>=targetRank)return res.status(403).json({error:'target_scope_denied'});next();}
function capabilityGuard(name){return(req,res,next)=>{const caps=CAPABILITIES[req.adminRow?.role]||{};if(!caps[name])return res.status(403).json({error:`capability_${name}_denied`});next();};}
...
express.application.get=function(path,...handlers){
  ...
  if(path==='/api/admin/users'){const first=handlers[0],last=handlers[handlers.length-1];return originalGet.call(this,path,first,requireAdmin,capabilityGuard('manageUsers'),last);}
  ...
  if(path==='/api/admin/role')return originalGet.call(this,path,...handlers.slice(0,Math.max(0,handlers.length-1)),(req,res,next)=>{const oldJson=res.json.bind(res);res.json=(body)=>{if(body&&body.role)body.capabilities=rolePayload(body.role);return oldJson(body);};next();},...handlers.slice(-1));
  return originalGet.call(this,path,...handlers);
};
express.application.post=function(path,...handlers){
  if(path==='/api/admin/login')return originalPost.call(this,path,...handlers.slice(0,Math.max(0,handlers.length-1)),(req,res,next)=>{const oldJson=res.json.bind(res);res.json=(body)=>{if(body&&body.token){try{const claims=jwt.verify(body.token,JWT_SECRET),row=activeRole(Number(claims.sub));if(row)body.token=jwt.sign({sub:row.user_id,wethaqId:row.wethaq_id,admin:true,role:row.role,codeVersion:Number(row.code_version||1)},JWT_SECRET,{expiresIn:'30d'});}catch{}}return oldJson(body);};next();},...handlers.slice(-1));
  if(path==='/api/admin/assign')return originalPost.call(this,path,...handlers.slice(0,1),capabilityGuard('assign'),assignmentIdentityGuard,...handlers.slice(1));
  if(path==='/api/admin/remove-role')return originalPost.call(this,path,...handlers.slice(0,1),capabilityGuard('removeRole'),nonFounderTargetGuard,targetManagementGuard,...handlers.slice(1));
  if(path==='/api/admin/rbac/ban')return originalPost.call(this,path,...handlers.slice(0,1),capabilityGuard('manageUsers'),(req,res,next)=>{const minutes=Math.max(0,Number(req.body?.minutes||0)),caps=CAPABILITIES[req.adminRow?.role]||{};if(minutes>0&&!caps.banTemporary)return res.status(403).json({error:'temporary_ban_denied'});if(minutes===0&&!caps.banPermanent)return res.status(403).json({error:'permanent_ban_denied'});next();},...handlers.slice(1));
  if(path==='/api/admin/unban')return originalPost.call(this,path,...handlers.slice(0,1),capabilityGuard('unban'),nonFounderTargetGuard,targetManagementGuard,...handlers.slice(1));
