...
express.application.get=function(path,...handlers){
  if(path==='/api/admin/policy')return originalGet.call(this,path,requireAdmin,injectRolePayload,...handlers);
  if(path==='/api/admin/structure')return originalGet.call(this,path,...handlers);
  if(path==='/api/admin/audit-log')return originalGet.call(this,path,...handlers.slice(0,Math.max(0,handlers.length-1)),requireAdmin,(req,res,next)=>{if(req.adminRow.role!=='founder')return res.status(403).json({error:'founder_only'});next();},...handlers.slice(-1));
  ...
  if(path==='/api/messages/inbox'||path==='/api/messages/:wethaqId')return originalGet.call(this,path,...handlers.slice(0,Math.max(0,handlers.length-1)),reshapeMessages(50),...handlers.slice(-1));
...
express.application.post=function(path,...handlers){
  if(path==='/api/identity'||path==='/api/login')return originalPost.call(this,path,userLoginGuard,...handlers);
...
express.application.listen=function(...args){
  const app=this;
  if(!app.__wethaqTrustRoute){
    app.__wethaqTrustRoute=true;
    originalGet.call(app,'/api/trust/:wethaqId',(req,res)=>{const u=db.prepare('SELECT wethaq_id,name,created_at,last_seen,avatar_data FROM users WHERE wethaq_id=?').get(req.params.wethaqId);if(!u)return res.status(404).json({error:'user_not_found'});res.json({trusted:true,wethaq_id:u.wethaq_id,name:u.name,created_at:u.created_at,online:false,has_avatar:!!u.avatar_data,trust_version:'1'});});
  }
